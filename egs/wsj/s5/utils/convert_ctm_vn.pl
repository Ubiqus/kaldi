...
# modified version of original to store utt-ID rather than file ID for regenerating the text file
  if (defined $conf) {
#    $line = "$file $channel $wbegin_r $wlen $w $conf\n";
    $line = "$utt $one $wbegin_r $wlen $w $conf\n";  
  } else {
#    $line = "$file $channel $wbegin_r $wlen $w\n";
    $line = "$utt $one $wbegin_r $wlen $w\n";  
  }
